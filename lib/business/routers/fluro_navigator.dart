...
class NavigatorUtils {
  ...
  static void push(BuildContext context, String path, {
    ...
    Routers.router.navigateTo(context, path, replace: replace,
  ...
  static void pushResult(BuildContext context, String path,
    ...
    Routers.router.navigateTo(context, path, replace: replace,
        clearStack: clearStack,
        transition: TransitionType.native).then((Object result) {
      // 页面返回result为null
      if (result == null) {
        return;
      }
  ...
  static void goWebViewPage(BuildContext context, String title, String url) {
    //fluro 不支持传中文,需转换
    //unfocus();
    push(context, '${WebRouter.webViewPage}?title=${Uri.encodeComponent(title)}&url=${Uri.encodeComponent(url)}');
  }
